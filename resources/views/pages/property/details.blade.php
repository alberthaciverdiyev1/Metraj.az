    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="space-y-3">
                ...
                <div class="relative w-full h-[320px] sm:h-[420px] md:h-[480px] lg:h-[500px] rounded-2xl md:rounded-3xl overflow-hidden bg-gray-900 shadow-sm select-none group">
                    @if($totalImages > 0)
                    ...
                    <div class="absolute bottom-4 left-4 flex items-center gap-1.5 z-10">
                        <div class="flex items-center gap-1.5 bg-white px-2.5 py-1 rounded-lg shadow-sm">
                            ...
                            <span class="w-4 h-4 rounded bg-red-600 text-white text-[10px] font-black flex items-center justify-center leading-none">V</span>
                        </div>
                    </div>

                    <!-- Bottom-Center Image Counter -->
                    <div id="hero-counter" class="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md text-white text-xs font-semibold px-3 py-1.5 rounded-full z-10 shadow-sm">
                        1/{{ max(1, $totalImages) }}
                    </div>
                ...
                <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                    ...
                    @foreach($visibleImages as $index => $image)
                        @if($hasMore && $index === ($maxThumbs - 1))
                            ...
                            <div class="h-16 sm:h-20 rounded-xl overflow-hidden cursor-pointer relative border-2 border-transparent hover:opacity-95 transition"
                                 onclick="openModal({{ $index }})">
                                <img src="{{ $image->url }}" alt="{{ $property->title }}" class="w-full h-full object-cover">
                                <div class="absolute inset-0 bg-black/65 hover:bg-black/55 backdrop-blur-[1px] flex items-center justify-center text-white text-xs sm:text-sm font-bold transition">
                                    +{{ $totalImages - $maxThumbs + 1 }} {{ __('şəkil') }}
                                </div>
                            </div>
                        @else
                            <div class="h-16 sm:h-20 rounded-xl overflow-hidden cursor-pointer relative border-2 hero-thumbnail {{ $index === 0 ? 'border-orange-500 ring-2 ring-orange-500/20' : 'border-transparent' }} hover:border-orange-300 transition"
            ...
            <div class="bg-white p-6 sm:p-7 rounded-3xl shadow-sm border border-gray-100 space-y-4">
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                    ...
                    <div class="flex items-center gap-3 text-gray-400 text-lg shrink-0">
                        <button class="w-9 h-9 rounded-xl bg-gray-50 hover:bg-red-50 hover:text-red-500 flex items-center justify-center transition" title="{{ __('Seçilmişlərə əlavə et') }}"><i class="bi bi-heart"></i></button>
                        <button class="w-9 h-9 rounded-xl bg-gray-50 hover:bg-orange-50 hover:text-orange-500 flex items-center justify-center transition" title="{{ __('Müqayisə et') }}"><i class="bi bi-arrow-left-right"></i></button>
                        <button class="w-9 h-9 rounded-xl bg-gray-50 hover:bg-gray-100 hover:text-gray-900 flex items-center justify-center transition" id="printBtn" title="{{ __('Çap et') }}"><i class="bi bi-printer"></i></button>
                        <button class="w-9 h-9 rounded-xl bg-gray-50 hover:bg-blue-50 hover:text-blue-500 flex items-center justify-center transition" title="{{ __('Paylaş') }}"><i class="bi bi-share"></i></button>
                    </div>
                </div>
        ...
        <div class="lg:col-span-4 self-stretch">
            <div class="sticky top-20 lg:top-[88px] z-20 space-y-3.5">
                ...
                <div class="bg-white rounded-3xl p-6 sm:p-7 shadow-sm border border-gray-100/90 flex flex-col justify-between space-y-5">
                    ...
                    <!-- Price Header -->
                    <div class="space-y-1">
                        <div class="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight flex items-baseline gap-1.5">
                ...
                <!-- Promotion Action Cards Row (3 Cards: İrəli çək, VIP et, Premium) -->
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <div id="btn-vip"
                         class="bg-white hover:bg-gray-50 border border-gray-200/90 rounded-2xl p-3 flex flex-col justify-between cursor-pointer transition shadow-2xs">
                        <div class="flex items-center justify-between">
                            <span class="text-xs sm:text-sm font-bold text-gray-800">{{ __('VIP et') }}</span>
                            <span class="w-4 h-4 rounded bg-red-600 text-white text-[10px] font-black flex items-center justify-center">V</span>
                        </div>
                        <span class="text-[11px] font-semibold text-blue-600 mt-1">5 AZN-dən</span>
                    </div>
...
<div id="modal" class="fixed inset-0 bg-black/95 z-[99999] flex flex-col justify-center items-center select-none" style="display: none;">
    <div class="absolute top-4 left-1/2 -translate-x-1/2 w-11/12 max-w-6xl flex justify-between items-center text-white z-[100002] px-2">
        <span id="counter" class="text-sm font-semibold bg-black/50 px-3.5 py-1.5 rounded-full backdrop-blur-md">1/{{ max(1, count($galleryImages)) }}</span>
        <div class="flex items-center gap-3">
            <button type="button" onclick="toggleFullscreen()" class="w-10 h-10 rounded-full bg-white/15 hover:bg-white/30 text-white flex items-center justify-center transition cursor-pointer" title="{{ __('Tam ekran') }}"><i class="bi bi-fullscreen text-sm"></i></button>
            <button type="button" onclick="closeModal()" class="w-10 h-10 rounded-full bg-white/15 hover:bg-white/30 text-white flex items-center justify-center transition cursor-pointer" title="{{ __('Bağla') }}"><i class="bi bi-x-lg text-sm"></i></button>
        </div>
    </div>
    <div class="absolute inset-0 flex items-center justify-between pointer-events-none z-[100000] px-2 sm:px-6">
        <button type="button" onclick="prevImage()" class="pointer-events-auto w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-black/50 hover:bg-orange-500 text-white flex items-center justify-center text-xl sm:text-2xl transition cursor-pointer backdrop-blur-sm"><i class="bi bi-chevron-left"></i></button>
        <img id="modal-image" src="" alt="Modal Image" class="pointer-events-auto max-w-[90vw] max-h-[75vh] object-contain rounded-2xl shadow-2xl">
        <button type="button" onclick="nextImage()" class="pointer-events-auto w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-black/50 hover:bg-orange-500 text-white flex items-center justify-center text-xl sm:text-2xl transition cursor-pointer backdrop-blur-sm"><i class="bi bi-chevron-right"></i></button>
    </div>
    <div class="absolute bottom-4 left-1/2 -translate-x-1/2 w-11/12 max-w-4xl flex space-x-2 overflow-x-auto p-2 bg-black/60 backdrop-blur-md rounded-2xl z-[100001] justify-center" id="thumbnails">
...
<script>
    window.galleryImages = @json($galleryImages->pluck('url'));
