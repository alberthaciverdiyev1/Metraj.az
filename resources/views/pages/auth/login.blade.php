...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async function (e) {
            ...
            submitBtn.innerHTML = '<span class="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span> <span>' + '{{ __("Yoxlanılır...") }}' + '</span>';
            ...
            try {
                ...
                if (response.ok && data.success) {
                    if (window.Metraj && window.Metraj.toast) {
                        window.Metraj.toast(data.message || '{{ __("Uğurla daxil oldunuz!") }}', 'success');
                    }
                    submitBtn.innerHTML = '<i class="bi bi-check2-circle text-lg"></i> <span>' + '{{ __("Daxil olundu!") }}' + '</span>';
                    ...
                    const msg = data.message || '{{ __("E-poçt və ya şifrə yanlışdır.") }}';
                    if (window.Metraj && window.Metraj.toast) {
