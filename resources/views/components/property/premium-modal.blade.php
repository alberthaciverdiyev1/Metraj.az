<div id="modal-premium"
    ...
    <div class="bg-white rounded-lg shadow-lg max-w-sm w-full mx-4 min-h-[50vh] max-h-[80vh] relative p-4 flex flex-col space-y-4">
        ...
        <p class="text-sm">{{ __('Elan bütün və Köhnə tikili almaq elanlarının axtarış nəticələrinin içində birinci yerə çıxacaq.') }}</p>
        <h4 class="font-medium mt-4">{{ __('XİDMƏTİN MÜDDƏTİNİ SEÇİN') }}</h4>
        <div class="flex flex-col space-y-2.5">
            <label class="flex items-center justify-between p-3 border rounded hover:bg-gray-50 cursor-pointer text-sm">
                <span>1 {{ __('dəfə') }} / 7,00 AZN</span>
                <input type="radio" name="premiumOption" class="h-4 w-4 text-blue-600">
            </label>
            <label class="flex items-center justify-between p-3 border rounded hover:bg-gray-50 cursor-pointer text-sm">
                <span>3 {{ __('dəfə') }} ({{ __('hər 24 saat') }}) / 14,00 AZN</span>
                <input type="radio" name="premiumOption" class="h-4 w-4 text-blue-600">
            </label>
            <label class="flex items-center justify-between p-3 border rounded hover:bg-gray-50 cursor-pointer text-sm">
                <span>5 {{ __('dəfə') }} ({{ __('hər 24 saat') }}) / 21,00 AZN</span>
                <input type="radio" name="premiumOption" class="h-4 w-4 text-blue-600">
            </label>
            <label class="flex items-center justify-between p-3 border rounded hover:bg-gray-50 cursor-pointer text-sm">
                <span>10 {{ __('dəfə') }} ({{ __('hər 24 saat') }}) / 28,00 AZN</span>
                <input type="radio" name="premiumOption" class="h-4 w-4 text-blue-600">
            </label>
        </div>
        <div class="border-t pt-3 flex flex-col space-y-2.5">
            <p class="text-xs text-gray-600">{{ __('«Ödə» düyməsini sıxmaqla siz platformanın İstifadəçi razılaşmasını və Ofertanı qəbul etdiyinizi təsdiq etmiş olursunuz.') }}</p>
            <button class="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold py-2 px-3 rounded">{{ __('Ödə') }}</button>
        </div>
    </div>
</div>
