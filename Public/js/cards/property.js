...
<div class="absolute top-3 left-3 flex flex-wrap  gap-2">
    ${property.add_type === 'rent' ? `<span class="bg-[color:var(--primary)] text-white text-xs font-semibold px-2 text-center py-1 rounded-full">Kirayə</span>` :
            property.add_type === 'sale' ? `<span class="bg-[#80807F] text-white text-xs font-semibold px-2 py-1 rounded-full">Satışda</span>` : ''}

    ${property.property_condition === 'REPAIRED' ? `<span class="bg-green-600 text-white text-xs font-semibold px-2 text-center  py-1 rounded-full">Təmirli</span>` :
            `<span class="bg-gray-400 text-white text-xs font-semibold px-2 py-1 text-center rounded-full">Təmirsiz</span>`}

    ${property.in_credit ? `<span class="bg-blue-600 text-white text-xs font-semibold px-2 py-1 text-center  rounded-full">İpoteka var</span>` :
            `<span class="bg-gray-400 text-white text-xs font-semibold px-2 py-1 text-center rounded-full">İpotekasız</span>`}

    ${property.document === 'kupça var' ? `<span class="bg-green-500 text-white text-xs font-semibold px-2 py-1 text-center  rounded-full">Çıxarışlı</span>` :
        ...
        <div onclick="window.location.href='/property/${property.id}'" 
            ...
            <div class="p-4 flex flex-col flex-1">
                <div class="flex flex-col gap-2" style="min-height:120px;">
                    <h3 class="font-bold text-[color:var(--text-color)] h-[30px] transition hover:text-[color:var(--primary)]">
                        ${property.title}
                    </h3>
